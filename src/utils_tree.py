...
class Tree():
    ...
    def draw(self):
        ...
        while len(q) > 0:
            cur = q.pop(0)
            print (self.hyper2hypo_edgeonly[cur],end=" ")
    ...
    def evaluate(self, data=None, micro_data=None, wrong_at_data=None, reward_type='', output=False, return_all=False,
        ...
        if output or reward_type == 'print_each':
            ...
            print ("Hyper-Prec: {} / {} = {:.3f}".format(sum(res_isHyper_Test), len(res_isHyper_Test), hyper_prec),end=" ")
            print ("Hyper-recall: {} / {} = {:.3f}".format(sum(res_isHyper_Test), len(self.hypo2hyper_set),
                                                          hyper_recall),end=" ")
            print ("Hyper-F1: {:.3f}".format(hyper_f1),end=" ")
            print( "Edge-Prec: {} / {} = {:.3f}".format(sum(res_isEdge_Test), len(self.taxo_test), edge_prec), end=" ")
            print ("Edge-recall = {} / {} = {:.3f}".format(sum(res_isEdge_Test), len(self.taxo) - 1, edge_recall),end=" ")
...
def read_tree_file(in_name, given_root, filter_root=False, allow_up=True, noUnderscore=False):  #Block 1.2.4
    trees = []
    ...
    with open(in_name) as f:
        ...
        for line in f:
            ...
            #print('in MCREL utils', 'realroot',realroot, 'hypo2hyper_edgeonly',hypo2hyper_edgeonly, 'given_root',given_root, 'filter_root',filter_root,
             #        'term_count',term_count,'allow_up',allow_up)
    return trees
